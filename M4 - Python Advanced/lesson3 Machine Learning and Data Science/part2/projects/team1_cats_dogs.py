#!/usr/bin/env python3
"""
Team 1 — Cats vs Dogs (Image Classification)

Dataset: Bingsu/Cat_and_Dog   (Hugging Face)
Modality: image (binary)

Goal
----
Classify cat vs dog images. Target: ≥94–96% test accuracy with a small CNN.

Suggested path
--------------
- Tiny CNN (2–3 conv blocks) or transfer learning with a frozen backbone (e.g., resnet18) + linear head.
- Use simple augmentation (random horizontal flip, small random crop).
- Report accuracy and confusion matrix; include 6–8 sample predictions.

Stretch ideas
-------------
- Unfreeze last backbone block for 1–2 epochs with a low learning rate.
- Add label-smoothing or mixup (optional).
...
def load_data(dataset_id: str = "Bingsu/Cat_and_Dog"):
